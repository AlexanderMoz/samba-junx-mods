...
again:
	...
	while (cnt_usr < IDMAP_LDAP_MAX_IDS &&
		...
		case ID_TYPE_UID:
			/* 
			 * For stand-alone LDAP server, escape the filter value to prevent LDAP injection attacks.
			 * Active Directory servers handle this internally, so escaping is only needed for
			 * standalone LDAP configurations (ldap_server = stand-alone).
			 */
			if (ctx->smbldap_state != NULL) {
				char *escaped_name = escape_ldap_string(mem_ctx, map->name);
				if (!escaped_name) {
					ret = NT_STATUS_NO_MEMORY;
					goto out;
				}
				fltr_usr = talloc_asprintf_append_buffer(fltr_usr,
					     "(%s:caseExactMatch:=%s)", (ctx->user_cn ? "cn" : "uid"),
					      escaped_name);
				talloc_free(escaped_name);
			} else {
				/* AD mode - no escaping needed as AD handles this internally */
				fltr_usr = talloc_asprintf_append_buffer(fltr_usr,
		...
		case ID_TYPE_GID:
			...
			if (ctx->smbldap_state != NULL) {
				...
				fltr_grp = talloc_asprintf_append_buffer(fltr_grp,
						 "(cn:caseExactMatch:=%s)", escaped_name);
				talloc_free(escaped_name);
			} else {
				/* AD mode - no escaping needed as AD handles this internally */
				fltr_grp = talloc_asprintf_append_buffer(fltr_grp,
