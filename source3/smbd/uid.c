	...
	if (vuid != UID_FIELD_INVALID) {
		...
		for (i=0; i<VUID_CACHE_SIZE; i++) {
			...
			if (ent->vuid == vuid) {
				...
				if (ent->session_info) {
					maybe_refresh_session_token(ent->session_info);
				}
...
bool change_to_user_and_service(connection_struct *conn, uint64_t vuid)
{
	...
	/* Check if we need to refresh the token before impersonating */
	if (si) {
		maybe_refresh_session_token(si);
	}
...
bool become_user_without_service(connection_struct *conn, uint64_t vuid)
{
	...
	/* Check if we need to refresh the token before impersonating */
	if (session_info) {
		maybe_refresh_session_token(session_info);
	}
