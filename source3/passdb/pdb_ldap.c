					  ...
					  const char **attr)
{
	 ...
	 */
	filter = talloc_asprintf(talloc_tos(), "(&%s%s)", "(uid:caseExactMatch:=%u)",
...
static NTSTATUS ldapsam_add_sam_account(struct pdb_methods *my_methods, struct samu * newpwd)
{
	...
	filter = talloc_strdup(attr_list, "(uid:caseExactMatch:=%u)");
	if (!filter) {
		...
		for (memberuid = values; *memberuid != NULL; memberuid += 1) {
			...
			if (escape_memberuid == NULL) {
				...
				goto done;
					...
					filter = talloc_asprintf_append_buffer(filter, "(uid:caseExactMatch:=%s)", escape_memberuid);
					       ...
					       uint32_t *p_num_groups)
{
	...
	*pp_sids = NULL;
	...
	if (user->unix_pw) {
		...
		filter = talloc_asprintf(mem_ctx,
					 "(&(objectClass="LDAP_OBJ_SAMBASAMACCOUNT")(uid:caseExactMatch:=%s))",
					 escape_name);
		if (filter == NULL) {
			ret = NT_STATUS_NO_MEMORY;
			goto done;
				    ...
				    uint32_t acb_info, uint32_t *rid)
{
	...
	filter = talloc_asprintf(tmp_ctx, "(&(uid:caseExactMatch:=%s)(objectClass="LDAP_OBJ_POSIXACCOUNT"))",
				 username);
					 ...
					 uint32_t *rid)
{
	...
	filter = talloc_asprintf(tmp_ctx, "(&(cn:caseExactMatch:=%s)(objectClass="LDAP_OBJ_POSIXGROUP"))",
				 groupname);
